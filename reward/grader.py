...
def grade_access_decision(world_state: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {"score": round(total, 4), "breakdown": scores, "weights": weights, "details": details}
...
def grade_jit_escalation(world_state: Dict[str, Any]) -> Dict[str, Any]:
    ...
    approval_chains = world_state.get("approval_chains", {})
    ...
    if not req:
        return {"score": 0.0, "breakdown": scores, "details": {"error": "No request found"}}
    ...
    elif len(routed_ids) > 0 and routed_ids[0] == correct_chain[0]:
        scores["correct_routing_order"] = 0.5
    details["routing_order"] = {"agent": routed_ids, "correct": correct_chain}

    # 3. Ticket attached (non-empty ticket_id in request)
    if req.get("ticket_id"):
        scores["ticket_attached"] = 1.0
    details["ticket"] = {"ticket_id": req.get("ticket_id")}

    # 4. Correct role (check _agent_ttl via access.set_ttl)
    ...
    if grant_activated and should_approve:
        ...
        scores["correct_final_decision"] = 1.0
    details["final_decision"] = {"grant_activated": grant_activated, "should_approve": should_approve}
...
def grade_access_review(world_state: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # 3. Workflow preservation: did agent revoke critical entitlements?
    broken = revoked_set & workflow_critical
    if not broken:
        scores["workflow_preservation"] = 1.0
    else:
        # Partial penalty per broken workflow entitlement
        scores["workflow_preservation"] = max(0.0, 1.0 - (len(broken) * 0.25))
    details["workflow_preservation"] = {
        "broken_entitlements": list(broken),
        "critical_count": len(workflow_critical),
    }

    # 4. Policy compliance: check remaining entitlements against policies
    ...
    for e in remaining.values():
        ...
        if matching_policy:
            ...
            if ROLE_RANK.get(e["role"], 0) > ROLE_RANK.get(max_role, 3):
                violations += 1
    if checked > 0:
        scores["policy_compliance"] = max(0.0, 1.0 - (violations / checked))
    else:
